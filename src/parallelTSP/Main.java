...
/**
			...
			**/
			...
			// Get parallel 2-opt tour info
			startTime = System.currentTimeMillis();
			parallelResult = parallelTwoOpt(nearestN, 50);
			length = Length.routeLength(parallelResult);
			time = System.currentTimeMillis() - startTime;
			System.out.println("Parallel 2-opt length(p=50) = 	" + length + " (" + time + "ms)");

			
			
			// Get parallel 2-opt tour info
			startTime = System.currentTimeMillis();
			parallelResult = parallelTwoOpt(nearestN, 100);
			length = Length.routeLength(parallelResult);
			time = System.currentTimeMillis() - startTime;
			System.out.println("Parallel 2-opt length(p=100) = 	" + length + " (" + time + "ms)");
	...
	private static ArrayList<Point2D> parallelTwoOpt2(ArrayList<Point2D> cities, int numThreads) {
		...
		while (improved) {
			for (int i = 0; i < numThreads; i++) {
				// parallelApproaches[i] = new ParallelTwoOpt(bestTour, numThreads);
				parallelApproaches[i].setBestTour(bestTour);
				threads[i] = new Thread(parallelApproaches[i], Integer.toString(i));
				threads[i].start();
			}
			iteration++;
			...
			// p.alternate(bestTour, numThreads);
		...
		while (improved) {
			...
			for (Future f : futures) {
				try {
					f.get();
					// futurecount ++;
					// System.out.println("Got future " + futurecount);
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
